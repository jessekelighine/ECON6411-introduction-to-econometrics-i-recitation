...
Throughout, the errors have finite second moments.

\begin{assumption}[Full Rank]\label{ass-full-rank}
	The design matrix $X$ has full column rank $k$.
\end{assumption}

\begin{assumption}[Conditional Mean Restriction]\label{ass-conditional-mean}
		$\E(e\given X)=0$.
\end{assumption}
...
\begin{proposition}[Conditional and Unconditional Unbiasedness]
	Under \autoref{ass-full-rank} and \ref{ass-conditional-mean},
	\begin{align}
...
\begin{theorem}[Gauss-Markov]\label{thm-gauss-markov}
	Under \autoref{ass-full-rank} and \ref{ass-conditional-mean} and $D=\sigma^2 I_n$,
	\gls{ols} is the best linear unbiased estimator.
\end{theorem}
\begin{proof}
...
\begin{remark}[What Gauss-Markov Does and Does Not Say]
	...
	When \eqref{eq-spherical-errors} fails,
	the sandwich formula \eqref{eq-sandwich} still applies under \autoref{ass-full-rank} and \ref{ass-conditional-mean}.
\end{remark}
...
\begin{proposition}[An Unbiased Covariance Estimator]
	Under \autoref{ass-full-rank} and \ref{ass-conditional-mean} and $D=\sigma^2 I_n$, the matrix
	\begin{align}
...
\begin{proposition}[\Gls{mse} for Prediction]
	Under \autoref{ass-full-rank} and \ref{ass-conditional-mean}, $D=\sigma^2 I_n$, and the assumptions above,
...
The variance comparison \eqref{eq-variable-variance-comparison} applies unchanged when $\beta_2=0$:
the covariance formulas do not depend on $\beta_2$.
Thus, including unnecessary variables cannot reduce the variance of any linear combination of the coefficients in $\beta_1$.
If $X_1$ and $X_2$ are not orthogonal, controlling for $X_2$ removes usable variation from $X_1$ and can increase variance.
...
\begin{proposition}[Ridge Bias and Variance]\label{prop-ridge}
	Under \autoref{ass-full-rank} and \autoref{ass-conditional-mean} and $D=\sigma^2 I_n$, for fixed $\lambda>0$,
...
\begin{remark}[Variance and \gls{mse} by Direction]
	Under \autoref{ass-full-rank} and \autoref{ass-conditional-mean} and $D=\sigma^2 I_n$, write $\theta_j=v_j\T\beta$.
	For fixed $\lambda>0$,
	\begin{align*}
